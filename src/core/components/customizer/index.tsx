...
const Customizer: React.FC<CustomizerProps> = (props) => {
  ...
  const handleTransitionChange = (option: any) => {
    setTransition(option.value)
  }

  // ** Render Layout Mode Options
  const renderLayoutModeRadio = () => {
    const modeArr = [
      {
        name: 'light',
        label: 'Light',
      ...
      },
      {
        name: 'semi-dark',
        label: 'Semi Dark',
        checked: layoutMode === 'semi-dark'
      }
    ]

    return modeArr.map((radio, index) => {
      const marginCondition = index !== modeArr.length - 1
      ...
      return (
        <CustomInput
          ...
          onChange={() => setLayoutMode(radio.name)}
      ...
      <PerfectScrollbar className="customizer-content">
        ...
        <div className="px-2">
          <FormGroup className="mb-2">
            <p className="font-weight-bold">Layout Mode</p>
            <div className="d-flex">{renderLayoutModeRadio()}</div>
          </FormGroup>
