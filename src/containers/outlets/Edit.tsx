...
const Edit = () => {
  ...
  const [territory, setTer] = useState<Territory | null>(null)
  const [values] = useState(() => {
    const { outlets } = store
    const item = outlets.find((o) => o.id === parseInt(id))
    const payload: OutletFields = {
      id: parseInt(id),
      ...
      subLocality: item ? item.subLocality : '',
      telephone: item ? item.telephone : '',
      territoryId: item ? item.territoryId : 0,
      coordinates: {
              ...
              <Form className="mt-2" onSubmit={handleSubmit}>
                <Row className="px-3">
                  <Col sm="12" md="12" lg="12">
                    ...
                    >
                      Update {values.outletName} for{' '}
                      {territory ? territory.locality : null} territory
                    </CardTitle>
                  </Col>
                </Row>
