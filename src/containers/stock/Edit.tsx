...
const Edit = () => {
  ...
  const [product, setPro] = useState<Product | null>(null)
  const [values] = useState(() => {
    const { stock } = store
    const item = stock.find((s) => s.id === id)
    const payload: StockFields = {
      unit: item ? { label: item.unit, value: item.unit } : '',
      comments: item ? item.comments : '',
      id: item ? item.id : '',
      productId: item ? item.productId : '',
      quantityPurchased: item ? item.quantityPurchased : '',
      reorderDate: item ? new Date(item.reorderDate) : new Date(),
      reorderLevel: item ? item.reorderLevel : '',
      reorderQuantity: item ? item.reorderQuantity : '',
      sku: item ? item.sku : '',
      unitPrice: item ? item.unitPrice : ''
    }
    item && dispatch(setProduct(item.product))
    return payload
  })
  ...
  const onSubmit = useCallback(
    (values: StockFields) => {
      ...
      dispatch(updateStockRequest(values))
    },
    [dispatch]
  )
              ...
              <Form className="mt-2" onSubmit={handleSubmit}>
                <Row className="px-3">
                  <Col sm="12" md="12" lg="12">
                    ...
                    >
                      Update stock for {product ? product.productName : null}
                    </CardTitle>
                  </Col>
                </Row>
